...
void replay_production_coin_HMS(int RunNumber=0, int MaxEvent=10000, int FirstEvent = 0, int MaxSegment = 0, int FirstSegment = 0, const char* fname_prefix = "nps_coin")
{
 ...
 // if (MaxEvent == 50000 && FirstEvent == 1){
  ...
  //}
  //else{
    ROOTFileNamePattern = "ROOTfiles/HMS/Boiling/nps_hms_coin_%d_%d_%d.root";
  //}
  
  
  // Add variables to global list.
  gHcParms->Define("gen_run_number", "Run Number", RunNumber); 
  gHcParms->AddString("g_ctp_database_filename", "DBASE/NPS/standard_coin_boil.database");  // FIXME: DBASE FOR HMS+NPS;
  ...
  analyzer->PrintReport("TEMPLATES/NPS/NPS_coin.template",
			Form("REPORT_OUTPUT/COIN/HMSONLY/coin_HMS_report_%d_%d.report", RunNumber, MaxEvent)); //FIXME:CHANGE
