...
void no_timing_windows_nps_hms_coin(int RunNumber=0, int MaxEvent=0, int FirstEvent = 1, int MaxSegment = 8, int FirstSegment = 0, const char* fname_prefix = "nps_coin") 
{
  ...
  if (MaxEvent == 50000){
    ROOTFileNamePattern = "ROOTfiles/NPS/50k/nps_hms_notiming_window_%d_%d.root";
  }
  else{
    ROOTFileNamePattern = "ROOTfiles/NPS/TIMING/nps_hms_notiming_window_%d_%d.root";
  }
  ...
  // Define crate map
  analyzer->SetCrateMapFileName("MAPS/NPS/CRATE/db_cratemap_coin.dat");
  ...
  analyzer->PrintReport("TEMPLATES/NPS/NPS_coin.template",
			Form("REPORT_OUTPUT/COIN/coin_NPS_HMS_report_notimewindow_%d_%d.report", RunNumber, MaxEvent)); //FIXME:CHANGE
}
