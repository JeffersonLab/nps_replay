...
// READ HERE !!!! THIS IS HARD CODED TO ONLY REPLAY 50k events!!! Ignore the MaxEvent = -1!!!!!!!!!
  ...
  // Define DEF-file+
  analyzer->SetOdefFile("DEF-files/NPS/ps_only.def");
  ...
  analyzer->PrintReport("TEMPLATES/NPS/ps_only.template",
  			Form("REPORT_OUTPUT/COIN/HMSONLY/scaler_report_%d.report", RunNumber)); //FIXME:CHANGE
