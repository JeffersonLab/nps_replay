  ...
  for(int ip=0; ip<NPLANES; ip++)
    {
      ...
      plane_dt_title_corr = Form("%s DC, Plane %s Corrected Drift Time",spec.Data(),plane_names[ip].Data());
      ...
      dt_vs_wire_title = Form("%s Drift Time vs. Wire: Plane %s",spec.Data(),plane_names[ip].Data());
      ...
      for (wire = 0; wire < nwires[ip]; wire++)
	{
	  ...
	  cell_dt_title = Form("%s DC Plane %s: Wire_%d",spec.Data(),plane_names[ip].Data(),wire+1);
	  ...
	  fitted_cell_dt_title = Form("%s DC Plane %s: Wire_%d",spec.Data(),plane_names[ip].Data(),wire+1);
	  ...
	  for (card = 0; card < plane_cards[ip]; card++ )
	    {
	      ...
	      card_hist_title = Form("%s DC Plane %s: Uncorrected Card_%d",spec.Data(),plane_names[ip].Data(),card+1);
	      ...
	      fitted_card_hist_title = Form("%s DC Plane %s: Fitted Card_%d",spec.Data(),plane_names[ip].Data(),card+1);
	      ...
	      corr_card_hist_title = Form("%s DC Plane %s: Corrected Card_%d",spec.Data(),plane_names[ip].Data(),card+1);
...
void DC_calib::WriteTZeroParam()
{
  ...
  otxtfile_name = Form("./%s_DC_%sLog_%s/%sdc_tzero_per_wire_%s.param",spec.Data(),mode.c_str(),std::to_string(run_NUM).c_str(),spectre.Data(),std::to_string(run_NUM).c_str());
...
void DC_calib::WriteLookUpTable()
{
  ...
  otxtfile_name	= Form("./%s_DC_%sLog_%s/%sdc_calib_%s.param",spec.Data(),mode.c_str(),std::to_string(run_NUM).c_str(),spectre.Data(),std::to_string(run_NUM).c_str());
  out_txtFILE.open(otxtfile_name);
...
//------------------------------------------------------------------------
	...
	itxtfile_name = Form("./%s_DC_%sLog_%s/t_zero_values_%s.dat",spec.Data(),mode.c_str(),std::to_string(run_NUM).c_str(),plane_names[ip].Data());
	  ...
	  for (int ip = 0; ip < NPLANES; ip++) 
	    {
	      ...
	      otxtfile_name = Form("./%s_DC_%sLog_%s/t_zeroCARD_values_%s.dat",spec.Data(),mode.c_str(),std::to_string(run_NUM).c_str(),plane_names[ip].Data());
	      out_txtFILE.open(otxtfile_name);
	      out_txtFILE << "#Plane_" + plane_names[ip] << endl;
